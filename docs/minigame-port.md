...
第 3 步的进度：**TD 与防守两种模式都能在小游戏里从头玩到结算，第一局的引导条也在**（§5.1–5.2.7），
还差**拖动 / 双指缩放**这一件，然后是 §6 那几项非代码门槛（AppID / 备案 / 类目 / 服务器 / 真机）。
...
* **现在能玩的**：点塔位 → 建造面板选塔种（5.2.1）、点已建的塔 → 塔面板升级/出售/优先级/修塔（5.2.1）、
  ...
  结算出来之后「开波」变成「再开一局」。
* **还没搬过来的**（浏览器版有）：**拖动战场与双指缩放**。两种模式都能进局（防守那屏见 5.2.6），
  第一局的引导条也在（见 5.2.7）；剩下这一件按「玩家每局都会碰一次」的顺序排在后面。
...
npm test                  # 298 项：小游戏这一层的大厅 / 战场 / 建造 / 商店 / 结算 / 暂停 / 继续上局 / 防守 / 新手引导
```
...
npm test                  # 298 项，含小游戏这几屏的布局 / 命中 / 状态迁移 / 记档
...
**这一屏还差的小项**（不影响能不能玩）：浮动摇杆开关（`settings.stick`，现在只有固定式）、

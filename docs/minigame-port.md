...
npm test                  # 350 项：小游戏这一层的大厅 / 战场 / 建造 / 商店 / 结算 / 暂停 / 继续上局 / 防守 / 新手引导 / 相机
```
...
npm test                  # 350 项，含小游戏这几屏的布局 / 命中 / 状态迁移 / 记档
...
| 4 | **自己去验证错误** | ✅ | 验证记录 §210–§259 全是小游戏这一路：每条都带「一个能失败的检查」+ 变异反证。`npm test` **350 项**（其中小游戏这一层 100+ 条：布局 / 命中 / 状态迁移 / 记档 / 两种模式的整局 / 四条通用几何扫描 / 动作接线 / id 唯一 / 模型对齐）· `npm run minigame` 6 条 · `npm run minigame:preview` 8 张样张 |

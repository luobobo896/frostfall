...
* **现在能玩的**：点塔位 → 建造面板选塔种（5.2.1）、点已建的塔 → 塔面板升级/出售/优先级/修塔（5.2.1）、
  ...
  结算出来之后「开波」变成「再开一局」。
* **还没搬过来的**（浏览器版有）：拖动与双指缩放、**防守模式**（跟随相机 + 摇杆 + 另一套 HUD）、新手引导条。
  防守那一格现在**明写着还没接**：大厅切到防守时「单人开局」会给出理由，而不是开一局 TD 糊弄过去。
  剩下这几屏按玩家每局都会走一遍的顺序：**防守 HUD（含摇杆）→ 引导条 → 拖动与双指缩放**。
...
npm run minigame:preview  # 本机 Chrome 出六张样张（docs/testing/screenshots/minigame-*.png）
npm test                  # 287 项，含小游戏这几屏的布局 / 命中 / 状态迁移 / 记档

...
npm test                  # 319 项：小游戏这一层的大厅 / 战场 / 建造 / 商店 / 结算 / 暂停 / 继续上局 / 防守 / 新手引导 / 相机
```
...
| 差距 | 现在是什么 | 要补的话 |
|---|---|---|
| **没有音效** | 小游戏这一版一个音都不放（`audio.js` 没进主包）。防守的「回防预警」在 HUD 上是一条 `⚠ Ns 后抵达` 的倒计时，看得见但听不见 | 接 `wx.createWebAudioContext` + 首次触摸解锁（浏览器版 §178 那套）；本地只能用假 `wx` 验到「调用了哪个接口」，听不听得见要真机 |
...
npm test                  # 319 项，含小游戏这几屏的布局 / 命中 / 状态迁移 / 记档
...
* **英雄那一格**：顶栏下面那行写「英雄 Lv1」，阵亡时改成红色的「阵亡 8s」。
  第一版把它塞进顶栏，结果跟「城堡 x/y」叠字（样张里一眼可见）——顶栏里轮次 + 预警 + 金 + 木 + 城堡
  已经占满，硬塞是不行的。
...
不落进摇杆区；阵亡时换成复活键、木材不够灰掉；顶栏那行写「阵亡 Ns」）与闭环
（防守局点技能真的进冷却、键上写剩余秒数；阵亡后木材不够点复活没用、够了真的花 50 木把人拉起来）。

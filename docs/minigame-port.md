...
npm test                  # 349 项：小游戏这一层的大厅 / 战场 / 建造 / 商店 / 结算 / 暂停 / 继续上局 / 防守 / 新手引导 / 相机
```
...
npm test                  # 349 项，含小游戏这几屏的布局 / 命中 / 状态迁移 / 记档
...
### 5.2.28 连打两局的跨局状态（已完成 · 2026-09-23）：防回归，不是抓到了 bug

浏览器版有一条第 §198 的检查（「连打两局 DOM 不长」）。小游戏这边没有对应的东西，
于是补一条**跨局状态**的：跳过引导 → 打完第一局 → 点「再开一局」→ 第二局必须是**新的**
（新对象、时间归零、金币回到开局值、没有上一局的结算）→ 第二局也打完 → 声望再涨一次
（一局记一次，§188）→ 回大厅那张图的卡面写成「通关 2/2」。

写这条时我第一版的期望是错的：以为第一局打完引导就该不再挂，结果第二局又挂了——
核对之后发现**挂是对的**（那一局我从头到尾没做过引导的几步，`tutorialDone` 一直是 false，
§153 的门槛只看它）。要测「不再挂」得先跳过或走完那四步，测试里改成先跳过。

**这条的状态**：它**没有抓到 bug**（现有实现是对的），价值在于把跨局那几个不变量钉住——
之后的改动如果让第二局继承了上一局的状态，它会红。

### 5.2.29 出门的文案（已完成 · 2026-09-23）：把「移植第 3 步」和内部 id 清干净

接着量「玩家会看到什么字」这一类，抓到两处**只有自己人看得懂**的：

* **大厅提示的前两行是开发进度**：`移植第 3 步：大厅 + 战场都跑在 Canvas 上` / `可点：模式 / 难度 / …`
  ——那是我们自己的状态注释，不该出现在玩家的屏幕上。换成三行**玩法提示**：
  `单人开局：守住核心，打完 12 波（长局 30 波）` / `防守生存：左下推杆走路 · 点工事位建塔 · 预警响了就回城` /
  `点英雄卡片再点一次，能看技能与天赋`（每行都在大厅那个 316 宽的提示框里放得下，有用例盯着）。
  顺带把开始按钮上那句残留的「单人开局（第 4 步接入）」也去掉——`canStart` 这道闸留着，但文案是中性的。
* **提示里出现内部 id**：建塔后的提示写着 `建了 tw_arrow`、改优先级写着 `优先级：front`——
  玩家不认识这些东西。现在写 `建了 箭塔` / `优先级：最靠前`（名字都从数据表取，与面板上那些按钮同一份）。

**验收**：`tests/minigame-lobby.test.js` 那条「画一帧」的用例**反过来**钉住这件事——
整帧文案里不许出现 `第 3 步` / `第 4 步` / `移植` / `还没接`（它以前正好钉的是「战斗还没接上」那半句）；
`tests/minigame-battle.test.js` 新增一条：建塔与改优先级的提示必须是中文名，
整帧里不许出现 `tw_arrow` 这类塔 id 或 `front` 这类枚举。两条变异反证都红。

### 5.2.30 回到前台要再申请一次屏幕常亮（已完成 · 2026-09-23）

§5.2.22 申请了屏幕常亮，但官方文档写的是：`wx.setKeepScreenOn` **只在当前小程序生效、离开就失效**。
也就是说玩家切出去接个电话、再切回来，屏幕可能又开始自己熄——而一局 TD 十几分钟、盯塔时未必碰屏幕。
现在 `wx.onShow`（浏览器是 `visibilitychange` 可见那一半，见 `platform.js` 的 `onShow`）
回来就再申请一次。

**验收**：`tests/minigame-battle.test.js` 那条平台杂项用例补一句——切后台再回前台，
`setKeepScreenOn(true)` 必须被**再调一次**（假 `wx` 也补了 `fireShow()`）。变异反证：拿掉那行，当场红。

### 5.2.31 无尽里的假出口 + 长局覆盖（已完成 · 2026-09-23）

* **§190 的假出口在小游戏里复活了**：`m.over`（城堡陷落）之后 `assault.endless` 仍然是 true，
  而这一版给「继续（无尽）」的判据只看了 `endless`——于是玩家会看到一颗点下去只是把面板让开、
  露出「城堡 0 血、怪站着不动」的死场的键。修法与浏览器版 `resultPanelModel.canContinue` 对齐：
  加上 `!m.over`。这条也是「§190 修过一次、移植时又漏了」的典型。
* **长局（30 波）在小游戏里没被验过**：大厅有「长局 30 波」那一格（TD 专属），
  但没有任何用例证明它真的按 30 波开。补一条：选长局 → 开局 → `m.waves.length === 30`、
  `m.length === 'long'`、顶栏那行写「30 波」。

**验收**：前者在「守住 4 轮转无尽」那条用例里加两步（先验 `over` 时**没有**那颗键——必须在面板被收起来之前验，
否则等于没验；再回到「刚守住」验它有），变异反证（去掉 `!m.over`）当场红；
后者是新用例，变异反证（把 `length` 写死成 short）红在「长局该是 30 波表（实际 12）」。

### 5.2.32 药品键：小药在冷却时改用那瓶没冷却的（已完成 · 2026-09-23）

同一个「照抄了实现、没照抄判据」的坑，这次在药品键上：浏览器版那颗键是**按包里的顺序挨个试**，
第一个放得出来的就用（`ui.js` 的 `btn.onclick` 里那个 `for`）。小游戏这边写死了「先小药、
没有小药才大药」——于是小药刚喝过、大药明明能用时，玩家会被小药挡住：包里躺着一瓶能救命的大药，
按下去却只得到一句「药品冷却中或没有药」。

现在与浏览器同一条顺序：`Object.keys(bag)` 里挨个试，用掉的那瓶名字写在提示里（`用了 大治疗药剂`）。
**验收**：`tests/minigame-battle.test.js` 一条——小药置成冷却中、两种药各一瓶，按一下必须用掉大药、
小药还在、提示写着用的是哪一瓶。变异反证（退回「先小药」）当场红。

### 5.2.33 防守第一次上手的那句提示（已完成 · 2026-09-23）

§14.3 稿 5 的防守操作行在浏览器里有一颗常驻键写着「点地面移动」，是**控制方式的说明**。
小游戏这边没有常驻操作行（摇杆区留给手），于是玩家第一次进防守时没有人告诉他「点地面也能走」——
而 §10.1 明确写了防守**同时支持摇杆与点地移动**两种操作，只发现一种等于少一半。

做法：开局给一句限时提示——`左下推杆走路 · 点地面也走 · 预警响了就回城`（4.5 秒），
**只给这张图还没打过的人**（`profile.clears` 里没有它就不念）。打过的人不必每次都被念一遍，
这也顺带说明为什么 `clears` 这个档案字段值钱。

**验收**：`tests/minigame-defense.test.js` 一条——第一次进这张防守图能在那帧里看到这句；
过几秒不再画（**数「有没有多画一次」**，因为记录是跨帧累积的）；打完一局记档之后重开这张图，
不再念。两条变异反证（整句不出现 / 每次都念）都红。
（写这条时踩了一个小坑：大厅那三行提示里也有「推杆走路」，而大厅那几帧画在同一张 canvas 上、
也在同一份记录里——用「推杆走路」去认会认成大厅那句。改用只在这句里出现过的「点地面也走」。）

### 5.2.34 正在操作的那个位置要高亮（已完成 · 2026-09-23）：两边都漏了一个参数

`render.js` 这两处一直支持「高亮当前操作对象」：
`drawIsoView` 认 `selectedSlot`（空塔位画绿圈）与 `selectedTower`（那一座塔描边），
`drawDefense` 认 `selectedFortSlot`（工事位变色）。浏览器版一直把它们传进去，
**小游戏以前写死成 null / 根本没传**——于是「点塔位 → 面板」「点工事位 → 面板」这两条路里，
画面上什么都没变，玩家得自己记住刚才点的是哪一个。

接上之后顺手发现浏览器版的一个小病：`view.selectedFortSlot` 是**只写不读**的老字段
（关轮盘时没人清），所以浏览器那边关掉轮盘之后那一格还亮着。改成渲染层直接问 UI
（`ui.openSlotIndex()` = 轮盘开着才有值），两边就都对了：**高亮只在面板开着时出现**。
那个只写不读的字段按删除纪律先扫了一遍引用（只剩写的那一处），删掉。

**验收**：小游戏两条——`tests/minigame-battle.test.js`（点空塔位 → 渲染器拿到 `selectedSlot`；
建完 → 拿到 `selectedTower`；关掉 → 都归 null）与 `tests/minigame-defense.test.js`
（点工事位 → `selectedFortSlot` 是那一格；关掉 → 归 null）。
浏览器那侧由 `smoke` 的 378 条守着（它走了一遍建围墙的完整路径）。
两条变异反证（把小游戏那两个参数写回 null）都红。

### 5.2.35 伤害飘字（已完成 · 2026-09-23）：打出去多少，看得见

§14.3 的战场稿上有飘字，浏览器版也一直有（`damageFloaters`：**相邻两帧的血量差**，
不依赖各自的伤害事件——联机也不下发伤害事件）。小游戏这一屏**一个飘字都没有**，
于是「这一发打掉多少」只能靠血条估。更要紧的是：§116 的「低特效档关飘字与脉冲」里，
飘字这半在小游戏里**根本没有可关的东西**（上一节刚把 effects 那一格补成真的，关的就只有脉冲）。

接法与这一路的习惯一致——**算法一行不重写**：

| 事 | 做法 |
|---|---|
| 算飘字 | 复用 `hud-model.js` 的 `damageFloaters(上一帧, 这一帧)`，每帧在 `drawFrame` 里做一次 |
| 时间基准 | 用**墙上时钟**（`Date.now()/1000`）而不是 `m.time`：暂停时 `m.time` 冻住，飘字会一直挂在屏幕上（浏览器那边用的也是墙上时钟） |
| 低特效档 | `settings.effects === 'low'` 时传空数组 —— 那一档关的就是飘字与脉冲（§116 现在两边都真的成立） |
| 过期 | 0.6 秒寿命、最多留 24 条（与浏览器同一套数字） |

**验收**：`tests/minigame-battle.test.js` 一条——建两座塔开波、**一秒一步地推并每步画帧**
（飘字看的是相邻两帧的差值，一口推 30 秒会把「出怪 → 被打 → 死」压在同一个间隔里，差值看不出来）
→ 飘字出现、写的是数字、真的传给了渲染器 → 切到低特效档之后传的是空数组 →
塞一条过期的进去、下一帧被收掉。两条变异反证（不收集 / 一律传空）都红。

### 5.2.36 两种模式各跑一局**真的打完**（已完成 · 2026-09-23）：小游戏版的 §189 / §190

浏览器冒烟有两条「门面话的底座」：§189 浏览器里**真的**把 12 波打完（参考打法、结算、记档、解锁），
§190 防守真的守满 4 轮转无尽。小游戏这边一直没有对应的——这一路所有「结算」都是**摆出来的**状态
（`m.result = 'win'`），也就是说「小游戏里能从头打到结算」这句话在小游戏侧**从没被完整跑过**。

现在补上两条，用的还是内核里那份参考打法（`ai.js` 的 `autoPlay` / `ai-defense.js` 的 `autoPlayDefense`，
浏览器 §189/§190 用的也是它们；它们内部调的 `update` / `updateDefense` 与 app 的 `tick()` 是同一份）：

* **TD**：大厅开局 → 参考打法打完（实测一局 4-20 分钟、12 波、≥8 座塔）→ 小游戏自己画一帧 →
  结算面板有「通关！」与「单局时长」→ 档案里声望 +120、`map_01` 记一胜、最快通关 = 本局 →
  **解锁 map_02**，回大厅能选到它；
* **防守**：大厅切防守 → 参考打法守到城堡陷落（一路经过「守满 4 轮 → 转无尽 → 无尽里陷落」）→
  `roundsCleared >= 4`、`assault.endless === true`、第 >4 轮、`result === 'win'` →
  把陷落摆成确定态再画一帧 → 面板写「守住了！」与「守住轮次」、**不再给「继续（无尽）」**（§190）→
  档案里 `def_01` 记一笔、`bestRounds >= 4`（§12.6 的防守战绩口径）。

两条加起来约 0.4 秒（内核算 12 波 + 21 分钟防守），比浏览器那两条还快。**这就是「小游戏里能玩」的底座**。

### 5.2.37 药品键的灰态（已完成 · 2026-09-23）：两瓶都在冷却时就该灰

同一类「判据只抄了一半」：浏览器版那颗药品键的灰态是
`Object.entries(m.potionCd).every(([, v]) => v > 0)`（**全都还在冷却**才灰），
小游戏只判了「有没有药」——于是两瓶刚喝完（都在冷却）时按钮还亮着，点下去只得到一句
「药品冷却中或没有药」。现在两边同一条判据：**包里还有一瓶没在冷却的**才亮（TD 底排与防守右排都改）。

**验收**：`tests/minigame-battle.test.js` 一条——有药且没冷却 → 能点；两瓶都在冷却 → 灰；
大药没冷却 → 能点（点了会用它，见 §5.2.32 那条顺序）；没药 → 灰。变异反证（把判据退回「有没有药」）当场红。

### 5.2.38 一条通用几何用例抓到的真 bug：工事面板的「墙」点不到（已完成 · 2026-09-23）

每条面板各自都有用例（建造 / 商店 / 暂停…），但每加一种新面板就漏一块——塔面板、背包、物品详情、
工事、**防守那套暂停行**都还没量过 §1.9.2 的热区与「不许出界」。于是补一条**通用的**：
遍历每一种面板的每一行，断言 ≥44、在画布内、在面板框里、互不重叠。

它第一次跑就抓到东西：**工事面板的两行完全叠在一起**——

```js
Object.values(FORTS).forEach((f, i) => rows.push({ ..., y: 96, ... }));   // `i` 收了却没用
```

后果不是「不好看」而是**功能性的**：`hitTestSheet` 返回第一个命中的行，所以两行重叠时
永远只命中上面那行——**「墙」这个工事在面板上点不到**，只能靠代码直接建（冒险者视角就是「没有这个选项」）。
原来的用例没抓到它：那条只查「两种工事都有行 + 买不起的灰掉」，行与行是不是压在一起它不管。

修法与其它面板一致（`y: 96 + i * 50`），并补一条**功能**用例：两行的 y 不同、墙那一行的命中是
`buildFort fort_wall`、真的点它之后墙上场。

同一把尺子接着扩到**跨块**：同一帧里同时出现的那几块（顶栏 / 胶囊区 / 下一波预告 / 英雄读数 /
引导条 / 结算面板 / 各种键）两两不许压在一起（浏览器那边 §145 就是这么查的）。它抓到防守
「击杀 N · 工事 n/m」那一行的尾巴塞进了倍速键的左下角（右边界 393 对键的左边界 380）——
现在那一行的右边界写进布局（`statsLine.right = 372`），英雄那一行的宽度也按它算。
那两行顺手垫了一层小面板：防守的战场会从下面跑过明亮地形，裸字读不清（对比度，不是被压）。

### 5.2.39 动作接线：界面给出的每一个动作，代码里都得有人接（已完成 · 2026-09-23）

第三把尺子——**接线**：界面里每一个能点的东西，背后都得有分支。新加一颗键、写了个新的
`action.type`，而 `applyAction` 里没有对应 case，它就静静落进 `default` 分支，**点下去什么都不发生**
（§3.1 #2 那条「不给假选项」说的正是这件事）。这种错在单个面板的用例里看不见：那些用例只查
「行在不在、灰态对不对」，不查「点了以后有没有人接」。

两条静态检查（`tests/minigame-actions.test.js`）：

* **正向**：把每种面板的每一行/每颗键的 `action.type` 收集起来（含大厅的卡片、英雄详情那层覆盖、
  攻城图上的「修塔」、防守专属的「摇杆 / 自动拾取」、结算那一帧的「再开一局」），
  逐个断言对应源码里存在 `case '<type>':`（大厅归 `lobby.js` 的 `applyLobbyAction`，
  其余归 `game.js` 的 `applyAction`）；
* **反向**：内核/界面里**确实有**的那些动作（35 个）也逐个断言在布局里出现过——防止「删了入口、
  留着分支」那种反向漂移。

同一条用例里还加了一把「**别把玩家困在面板里**」的尺子：每种面板都必须有出口
（关闭 / 取消 / 继续 / 回大厅 / 收起 / 关掉详情）。

**验收**：把商店的关闭行动作改成一个不存在的类型（`closeShop`），正向检查当场红并指名道姓
（`商店 → closeShop`）；把电梯……把关掉面板那一行删掉时，因为「继续游戏 / 回大厅」还在，
出口那条**不会**红（这是对的：它管的是「有没有出口」，不是「必须有某一颗键」）。

同一把尺子再加一格：**id 不许重复**。重复了 `Object.fromEntries` 只留最后一个，`byId` 就会指向
另一行——命中测试、用例、「点了哪一行」的判断**全都会指错人**；而这种错在单个面板的用例里同样
看不见（按 id 取到的仍是「某一行」）。检查：逐面板断言 id 唯一、`byId` 数量与 `rows` 一致（HUD 同理）。
变异反证：把「收起面板」那行的 id 改成 `resume`，当场红并列出重复的那两行。

**最后一格：模型与布局对账**。`skills`（买了技能书第三颗键不出现）与 `potionReady`（样张里药品键
画成灰的）都是同一族的：布局开始读某个字段，而某个调用点手拼的模型没跟上——字段 `undefined`，
那一格静默画成「不可用 / 空」，**不报错**。现在从 `layoutBattle` / `layoutDefense` 的源码里抠出所有
`model.X`，逐个断言它在模型构造器给的键里、或在一个写明白的白名单里（`paused` / `rate` / `tutorial` /
`ui` / `sheet`）；新字段必须做一次有意识的选择。
之后的改动如果让第二局继承了上一局的状态，它会红。

## 8. 完成度审计（2026-09-23）：目标「移植到能发布」逐条对证据

> 用途与浏览器侧那份（`docs/STATUS.md` §6）一样：**「做完了」不是一句话，而是下面每条都能复跑**。
> 审计对象是这轮任务的三件事——「按设计文档把界面搬到 Canvas」「一直开发到能玩」「自己去验证错误」，
> 外加「能不能发布到微信小游戏」。

| # | 要求 | 判定 | 证据（都能复跑） |
|---|---|---|---|
| 1 | **平台适配与打包** | ✅ | `src/platform.js` 一层收掉存储 / HTTP / socket / 音频 / 视口 / 生命周期 / 触摸 / 触觉 / 屏幕常亮 / 内存告警；`npm run minigame` 的「假 wx 下入口认出小游戏环境、存储往返成功」+「打包后的内核与源码逐字段一致（120 秒 / 种子 7）」 |
| 2 | **界面从 DOM 换到 Canvas（§14.3 十一稿 + 相机）** | ✅ | 逐屏记录见 §5.1–5.2.39：大厅 §5.1、战场 §5.2、建造与塔面板 §5.2.1、商店与背包 §5.2.2、结算 §5.2.3、暂停与设置 §5.2.4、继续上局 §5.2.5、防守 §5.2.6、引导条 §5.2.7、相机 §5.2.8、视口门槛 §5.2.9、英雄详情 §5.2.19、小地图 §5.2.16、技能键 §5.2.18、无尽出口 §5.2.15、飘字 §5.2.35 |
| 3 | **一直开发到能玩** | ✅ | **两种模式各有一条「真的打完」的用例**（§253）：TD 用参考打法打完 12 波 → 结算面板 → 记档 → 解锁 map_02；防守守满 4 轮 → 转无尽 → 城堡陷落 → 结算与记档。两条走的是内核里那份参考打法（`ai.js` / `ai-defense.js`），合计约 0.4 秒 |
| 4 | **自己去验证错误** | ✅ | 验证记录 §210–§259 全是小游戏这一路：每条都带「一个能失败的检查」+ 变异反证。`npm test` **349 项**（其中小游戏这一层 100+ 条：布局 / 命中 / 状态迁移 / 记档 / 两种模式的整局 / 四条通用几何扫描 / 动作接线 / id 唯一 / 模型对齐）· `npm run minigame` 6 条 · `npm run minigame:preview` 8 张样张 |
| 5 | **没碰坏浏览器那侧** | ✅ | `src/render.js` / `match.js` / `platform.js` 这几处共享文件的改动都是**加法**（`size` 口子、`fitText`、`REVIVE_LUMBER` 常量、`onShow`）；`npm run smoke` **378 条**全绿 |
| 6 | **能发布吗** | ⏳ **代码侧完成，卡在你那边** | 发布门槛全在 §6：AppID、备案、类目与资质、联机要的 wss + 已备案域名、真机验收。§6.1 是一步步的导入说明，§6.2 是**有意留着**的差距（音效只有预警一声、小屏明说「屏幕太小」、联机 / 分享 / 排行榜要外部条件、没有日志面板） |

**结论**：**代码这一侧没有已知未完成项**——十一稿与相机都在 Canvas 上、两种模式都能从头玩到结算、
错误都带能失败的检查。剩下的两项（§6 的发布门槛与真机验收）**必须在微信开发者工具与真机上做**，
本地只能验到「接口调对了、样张画出来了」这一层；哪一项过了/没过，回来告诉我，我按老规矩接着修。

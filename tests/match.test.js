...
test('§5.2 橙装的源头：精英 5% + 合成；第 12 波 Boss 那 15% 目前是死源头', () => {
  ...
  /**
   * §3.1 #2 拍板后，附录 B.1 那条「橙装 5-8 局出一次」已经**勾上**（实测 5.3 局一次）——
   * 勾上的线要有东西守着，所以这里两侧都卡（200 个种子是**定种子**样本，两个界都是确定性的，
   * 不会像旧版那样靠运气）。
   *
   * 顺带一个**当轮才量清的事实**（验证记录 §212）：把精英橙率改回 0.05，这条线**也过**
   * （6.9 局一次，仍落在 5-8 里）——#2 当初那句「实测 8-12 局一次」是**旧参考打法**下的数
   * （那会儿合成贡献比现在小）。所以这条断言守的是**设计带**，不是那个具体的橙率；
   * 0.05 → 0.08 的真正效果是「让精英而不是合成成为橙装的主要来源」（§5.2 的意图），频率上两者都在带内。
   */
  assert.ok(perGame >= 0.125 && perGame <= 0.2,
    `橙装 ${(1 / perGame).toFixed(1)} 局一次，掉出附录 B.1 的 5-8 局一次（实测口径 0.125-0.2 件/局）`);

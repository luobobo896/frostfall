// 附录清单里那条：「**技能书购买率 ≥ 30%**（若过低说明贵或没被看见；过高说明难度逼着人人必须买）」。
// §3.1 #25 拍板（秘传书 600 → 200）之后实测 38.9% ✅——这条线现在**由本工具守着**（低于 30% 就 exit 1）。
//
...
/**
 * 门槛（§3.1 #25 已决）：**购买率 ≥ 30%**——附录 B.1 那条当初就是按这个数写的，现在它有值可取了，
 * 就该由工具守着（§112 的「只报数」是因为当时这条线还挂在「等拍板」上：STATUS §3.1 拍完板、附录 B 勾上之后，
 * 它和 `matrix` / `defense-matrix` 一样是**有结论的线**）。
 */
const rate = (100 * withAny) / rows.length;
console.log(`\n验收检查（附录 B.1「技能书购买率 ≥ 30%」）：实测 ${pct(withAny, rows.length)}`);
if (rate < 30) {
  console.log(`❌ 低于线：${pct(withAny, rows.length)} < 30%（多半是又卖贵了，或者参考打法不买账了）`);
  process.exit(1);
}
console.log('✅ 达标（其余几行仍只报数：秘传 / 精研各买了多少、买得起却没买的局数）');
process.exit(0);
